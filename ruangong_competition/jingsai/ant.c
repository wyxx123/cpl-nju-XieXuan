...
bool JudgeCIsSafe(char moved_board[5][9], int* is_no_k);
...
{
	int is_no_k = 0;
	bool is_safe = JudgeCIsSafe(moved_board, &is_no_k);
 ...
 *
 * @param moved_board
 * @param is_no_k 0:is k, 1: no k, -1: no need to judge.
	...
	if (type == 'k')
		for (int k = 0; k < 4; k++)
		{
					...
					&& moved_board[new_i][new_j] != 'e')
			{
				...
				int tmp_input = -1;
				is_ant_win = !(JudgeCIsSafe(double_moved_board, &tmp_input));
			}
		}
